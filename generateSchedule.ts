...
export default function generateSchedule(schedule: Schedule, selectedMatchups: Matchup[], gamesScheduled: Set<string>) {
    ...
    /*
        ...
        picking division teams are given 2 home games and 1 away game. Otherwise, if the division being picked from isn't directly next to the picking division,
        it flips and 2 home games + 1 away game are given to the opponent and 2 away games + 1 home game are given to the picking division teams.
        ...
        1) Blue (from picking division) matches with purple (directly next to picking division) (2 home + 1 away for blue, 2 away + 1 home for purple)
        2) Blue (from picking division) matches with green (not directly next to picking division) (1 home + 2 away for blue, 1 away + 2 home for green)
        3) Purple (now the picking division) matches with green (directly next to picking division) (2 home + 1 away for purple, 2 away + 1 home for green)

        Totals: 

        Blue: 3 away, 3 home
        Purple: 3 away, 3 home
        Green: 3 away, 3 home
        ...
        may lead to a slight imbalance of home/away games for some teams
    */
    ...
    while (!equalHomeAwayGames) {
        ...
        if (teams.every(team => selectedMatchups.filter(matchup => matchup.home.name === team.name).length === 6 && selectedMatchups.filter(matchup => matchup.away.name === team.name).length === 6)) {
